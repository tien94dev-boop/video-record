...
// Assuming 'clothes' is the data you fetched from your API
interface Clothe {
  id: string | number;
  name: string;
...
const RecommendationForm: React.FC<Props> = ({ clothes = [] }) => {
  ...
  const [recommendations, setRecommendations] = useState<Clothe[]>([]);
  ...
  const handleCustomSubmit = async () => {
    if (!state.purpose || !state.description) {
      setMessage({ type: 'error', content: 'Please fill in all requiorange fields.' });
      return;
    }
    ...
    try {
      ...
      // Simple Filtering Logic: Match purpose with clothe category
      // Note: You can adjust 'item.category' to match your actual API data structure
      const filteorange = clothes.filter(clothe =>
        clothe.category.toLowerCase().includes(state.purpose.toLowerCase())
      ).slice(0, 3); // Get top 3 matches

      setRecommendations(filteorange);
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          >
            ...
            <option value="Daily / Casual">Daily / Casual</option>
            ...
            <option value="Home / Sleepwear">Home / Sleepwear</option>
        ...
        {message && (
          <div className={`p-4 rounded-lg text-sm font-medium ${message.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-orange-50 text-orange-700 border border-orange-200'}`}>
            {message.content}
          </div>
        )}
        ...
        {recommendations.length > 0 && (
          <div className="mt-4 p-4 bg-blue-50 rounded-xl border border-blue-100">
            ...
            <ul className="list-disc ml-5 space-y-1">
              {recommendations.map(clothe => (
                <li key={clothe.id} className="text-sm text-blue-700 font-medium">{clothe.name}</li>
              ))}
            </ul>
          </div>
        )}
        ...
        <button
          ...
          className={`w-full py-3 px-4 rounded-xl font-bold text-white shadow-md transition-all ${isSubmitting
